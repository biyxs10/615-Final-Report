...
As figure 4 shows, Palau's population decreased significantly from 2010 to 2013, reaching a minimum of 17,648 in 2013. Since then, it has slowly increased, but since 2019, there has been a slight decrease.
```{r}
print(population_plot)
```

As for the number of international tourists in figure 5, the overall trend from 2010 to 2015 was on the rise, reaching a peak of 162,000 people a year. It has decreased every year since then. The plunge in 2019-2020 due to the global epidemic is consistent with the downward trend in GDP.
```{r}
visitors_plot
```

For weather data, this report analyzes average annual rainfall and average annual temperature. Figure6 shows that the average annual rainfall in Palau from 2012 to 2021 is around 8 to 15inches. Figure 7 shows that the average annual temperature in Palau from 2012 to 2021 is relatively stable, around 80°F.
```{r}
prep_plot
temp_plot
```
...
comp_visitors<-ggplot(compare_data, aes(x = year, y = Visitors, color = country, linetype = country)) +
  ...
  theme_minimal()
```
The GDP comparison shows the economic trends of Fiji and Palau over the years. As figure 8 and 9 shows, Fiji's economy is significantly larger than Palau's due to its higher population and diversified economic sectors. Additionally, Fiji's GDP also fluctuated significantly overall.
```{r}
fiji_gdp
comp_gdp
```
